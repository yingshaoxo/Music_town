...
def music():
    ...
    if not os.path.exists(MUSIC_DIR):
        return ''
    files = [file for file in os.listdir(MUSIC_DIR) if os.path.isfile(os.path.join(MUSIC_DIR, file))]
    length = len(files)
    if length == 0:
        return ''
    return send_from_directory(MUSIC_DIR, files[random.randrange(0, length)])
...
def manage():
    ...
    if request.method == 'POST':
        ...
        if btn_value[:len('Delete')] == 'Delete':
            ...
            songs = read_music(username)
            try:
                os.remove(os.path.join(os.path.join(env.current_dir, 'static/music'), songs[delete_id]['name']))
                del songs[delete_id]
            except:
    ...
    if request.method == 'POST':
        ...
        for file in uploaded_files:
            ...
            if file and '.mp3' in file.filename:
                ...
                temp_name = os.path.join(os.path.join(env.current_dir, 'static/music'), '@w@{}'.format(filename))
                file.save(temp_name)           
                try:
